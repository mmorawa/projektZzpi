...
Następnie rozpoczeliśmy testy działania elementów strony internetowej, pierwszym testem było sprawdzenie poprawności skryptu odpowiadającego za logowanie i rejestracje pracowników. W tym celu utworzyliśmy kilku przykładowych pracowników i przeprowadzilismy próby logowania się na ich konta. Wraz z tworzeniem kont sprawdzona została poprawność działania systemu bazodanowego, który odpowiada za przechowywane dane pracowników i firmy.
...
Kolejnym elementem który przetestowaliśmy jest system rejestracji wejść i wyjść pracowników oraz wyświetlanie tabeli z pracownikami obecnymio na zakładzie.
<<Sprawozdanie z realizacji zadania (w tym ocena zgodności z estymatą). Kod programu (środowisko \texttt{verbatim}): \begin{verbatim}
for (i=1; i<10; i++)
...
\end{verbatim}>>.
