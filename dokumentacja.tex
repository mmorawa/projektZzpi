...
\subsection{Sprint Review/Demo}
Dnia 20.12.17 r. przedstawiono demo produktu w ramach drugiego Sprintu. Wszystkie założone cele zawarte w backlog itemach zaplanowanych na ten sprint zostały zrealizowane bez żadnych przeszkód. Prezentacja sprintu zawierała przedstawienie możliwności zalogowannia się na strone zakładową, strona udostepniała funkcje w zależności od uprawnień zalogowanego użytkownika. Kierownik, który dysponuje pełnymi prawami może zatrudniać oraz zwalniać pracowników oraz kontrolować podsumowania miesięczne przepracowanych godzin pracowników. Pracownik po zalogowaniu będzie mogł jedynie sprawdzić kto jest obecnie na zakładzie. Dla kierownika została dodana rowież funkcja dodawania nowych kart dla pracowników. Po przyłożeniu karty, która nie jest dodana do bazy danych, buzzer wyda dzwięk i karta dopiszę się do kart pustych. Jeżli przyłoży się tą kartę ponownie, wyświetli się komunikat że karta jest już dodana do bazy. Dodawanie nowych pracowników odbywa się poprzez wypełnienie formularza. Jest tam możliwość wyboru stanowiska oraz pustej karty z rozwijanej listy, pola z danymi osobowymi oraz miejsce wpisania loginu oraz hasła przyszłego pracownika, którym będzie mógł się logować na stronie. Po wykorzystaniu wszystkich pustych kart, nie będzie już możliwości dodania nowego pracownika. Zwalnianie pracowników odbywa się poprzez podanie ID pracownika. Dla ułatwienia odszukania ID danego pracownika, wszyscy są wyświetleni w tabeli niżej. Zwolnienie głównego kierownika jest zablokowane.
