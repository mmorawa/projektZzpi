...
\subparagraph{Realizacja.} W języku python został utworzony program, który za pomocą czytnika RFID odczytuje id kart i dodaje je do bazy danych (tabela Karty). Program ten sprawdza czy odczytane id istnieje już w tej tabeli czy też nie i wydaje odpowiednio sygnały buzzerem oraz diodami led: jeśli id nieistnieje zapala się zielona dioda wydawanyn jest jeden dłuższy sygnał a  po 3sekundach 2 krótsze a id jest dopisywane do bazy, w przeciwnym wypadku nadal pali się czerwona dioda a buzzer wydaje 5x krótki sygnał.
